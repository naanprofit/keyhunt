			...
			BSGS_AMP3[i].Reduce();
               ...
               if(use_mmap){
                      ...
                      if(bptable_size_override && bptable_size_override > map_bytes){
                       ...
                       FLAGBPTABLEMAPPED = 1;
                       /*
                        * Avoid zeroing the mapped region. Newly created files are already
                        * zero-filled, and the generation code overwrites every entry. Skipping
                        * the memset keeps creation and reload times manageable for multi-GB
                        * tables.
                        */
